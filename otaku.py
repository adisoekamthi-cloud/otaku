...
async def scrape_kuramanime():
    ...
    async with async_playwright() as p:
        ...
        # Set the user agent when creating a new context
        ...
        for anime in anime_list:
            ...
            episodes = await page.eval_on_selector_all(
                ...
                """nodes => {
                    ...
                    const lastEpisodeNode = nodes[nodes.length - 1];
                    return [{
                        episode: lastEpisodeNode.innerText.trim().replace(/\\s+/g, " "),
                        link: lastEpisodeNode.href
                    }];
                }"""
            )
            ...
            for ep in episodes:
                ...
                try:
                    response = requests.post("https://app.ciptakode.my.id/insertUpdate.php", json={
...
def run_flask():
    try:
        ...
        if "Address already in use" in str(e):
            print("⚠️ Port 3000 sudah digunakan. Silakan tutup program lain atau ganti port.")
        else:
            print("❌ Flask error:", e)

def run_scraper():
    loop = asyncio.get_event_loop()
    loop.run_until_complete(scrape_kuramanime())
